...
import FeedbackFormElement from 'cipherguard-mad/form/feedback';
import Form from "cipherguard-mad/form/form";
import Role from 'cipherguard-mad/test/model/map/role';
import TextboxFormElement from 'cipherguard-mad/form/element/textbox';
...
describe("Form", () => {
  ...
  it("load() should load the form with an instance of Model object", () => {
    ...
    const data = form.getData();
    expect(textbox.getValue()).to.be.equal('ada@cipherguard.com');
    expect(checkbox.getValue()).to.be.eql(['admin', 'user']);
    expect(data.User.username).to.be.equal('ada@cipherguard.com');
